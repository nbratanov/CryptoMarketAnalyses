...
class KNN_NLC_Classifer():
    ...
    def predict(self, x_test):
        ...
        for i in range(len(x_test)):
            ...
            if x_test[i + train_size + 1] != '' and x_test[i + train_size + 1] is not None:
                ...
                while(j < self.x_train.shape[0]):
                    if (j +1 >= self.x_train.shape[0]):
                        ...
                        document = self.x_test[i + train_size + 1]
            ...
            print(prediction)
            print(get_class(test_corpus.loc[train_size + i + 1, 'output']))
            print(test_corpus.loc[train_size + i + 1, 'output'])

            print(prediction == get_class(test_corpus.loc[train_size + i + 1, 'output']))
